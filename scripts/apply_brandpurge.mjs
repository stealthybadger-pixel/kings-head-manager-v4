// Deletes the 486 Booker/Urban brand-purge docs (Curtis, Sarsons, Heinz, Nestle, Pringles, etc.)
// from both the local emulator and, if --prod passed, live production.
// Usage: node scripts/apply_brandpurge.mjs           -> emulator
//        node scripts/apply_brandpurge.mjs --prod     -> production
...
async function run() {
  const ids = JSON.parse(fs.readFileSync('scripts/booker_urban_brandpurge_removal_ids.json', 'utf8'));
