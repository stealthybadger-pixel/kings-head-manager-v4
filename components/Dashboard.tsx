...
import { APPROVED_SUPPLIERS } from '../constants';
...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  const { ingredients, recipes, dishes, loading, updateIngredient } = useKitchenData();
  const [metric, setMetric] = useState<'value' | 'level'>('value');
  const [chartSupplier, setChartSupplier] = useState('ALL');
  ...
  const stockData = useMemo(() => {
    const calculated = ingredients
      .filter(i => {
        const matchesSupplier = chartSupplier === 'ALL' || i.suppliers.some(s => s.name === chartSupplier);
        const matchesCategory = chartCategory === 'ALL' || i.category === chartCategory;
    ...
    return calculated
      ...
      .slice(0, 20);
  }, [ingredients, metric, chartSupplier, chartCategory]);
  ...
  const handleBarContextMenu = (e: React.MouseEvent, data: typeof stockData[0]) => {
    e.preventDefault();
    const ing = ingredients.find(i => i.id === data.id);
    if (!ing) return;
  ...
  const handleContextSave = async () => {
    if (!contextMenu) return;
    const ing = ingredients.find(i => i.id === contextMenu.id);
    if (!ing) return;
    setIsSavingCtx(true);
    try {
      const updatedSuppliers = ing.suppliers.map(s =>
        (s.isPreferred || (!ing.suppliers.some(x => x.isPreferred) && s === ing.suppliers[0]))
          ? { ...s, name: contextMenu.supplier }
          : s
      );
      await updateIngredient(contextMenu.id, {
        category: contextMenu.category,
        suppliers: updatedSuppliers,
      });
      setContextMenu(null);
    } finally {
      setIsSavingCtx(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#111111] overflow-y-auto p-4 md:p-6 space-y-3 md:space-y-4 pb-24 max-w-7xl mx-auto w-full">
      ...
      <div className="border border-[#333333] bg-[#161616] flex-1 min-h-[400px] flex flex-col shadow-2xl relative overflow-hidden">
        <div className="p-2 md:p-3 border-b border-[#333333] flex flex-wrap justify-between items-center gap-2 bg-[#1c1c1c]">
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-[#c8a96e]">Inventory Analysis</h3>
            <p className="text-[8px] font-mono text-[#666] uppercase mt-0.5">Top 20 · ranked by {metric} · right-click bar to edit</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex flex-col">
              <label className="text-[8px] font-bold uppercase text-[#666] mb-0.5">Supplier</label>
              <select
                value={chartSupplier}
                onChange={e => setChartSupplier(e.target.value)}
                className="bg-[#1c1c1c] border border-[#333] px-2 py-1 text-[9px] text-[#e0e0e0] focus:outline-none focus:border-[#c8a96e] font-mono"
              >
                {chartSuppliers.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div className="flex flex-col">
              <label className="text-[8px] font-bold uppercase text-[#666] mb-0.5">Category</label>
              <select
                value={chartCategory}
                onChange={e => setChartCategory(e.target.value)}
                className="bg-[#1c1c1c] border border-[#333] px-2 py-1 text-[9px] text-[#e0e0e0] focus:outline-none focus:border-[#c8a96e] font-mono"
              >
                {chartCategories.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div className="flex border border-[#333333] bg-black self-end">
              <button
                ...
                {stockData.length > 0 ? stockData.map((data) => {
                  ...
                  return (
                    <div
                      ...
                      onContextMenu={e => handleBarContextMenu(e, data)}
                    >
                      <div className="opacity-0 group-hover:opacity-100 transition-all duration-200 bg-black border border-[#c8a96e] p-2 absolute z-40 top-2 left-1/2 -translate-x-1/2 pointer-events-none whitespace-nowrap shadow-[0_0_20px_rgba(0,0,0,0.8)]">
                          ...
                          <div className="text-[8px] font-mono text-[#666] mt-0.5">{data.category}</div>
                      </div>
      ...
      {contextMenu && (
        <div
          ...
          className="fixed z-[200] bg-[#1c1c1c] border border-[#c8a96e] shadow-2xl min-w-[230px]"
        >
          <div className="px-4 py-2.5 border-b border-[#333] bg-[#161616]">
            <div className="text-[8px] font-bold uppercase tracking-widest text-[#666]">Edit Ingredient</div>
            <div className="text-[11px] font-bold uppercase text-[#c8a96e] truncate mt-0.5">{contextMenu.name}</div>
          </div>
          <div className="p-4 space-y-3">
            <div>
              <label className="text-[8px] uppercase font-bold text-[#666] mb-1 block">Category</label>
              <select
                value={contextMenu.category}
                onChange={e => setContextMenu(m => m ? { ...m, category: e.target.value } : null)}
                className="w-full bg-[#111] border border-[#333] px-2 py-1.5 text-[10px] text-[#e0e0e0] focus:outline-none focus:border-[#c8a96e] font-mono"
              >
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[8px] uppercase font-bold text-[#666] mb-1 block">Supplier</label>
              <select
                value={contextMenu.supplier}
                onChange={e => setContextMenu(m => m ? { ...m, supplier: e.target.value } : null)}
                className="w-full bg-[#111] border border-[#333] px-2 py-1.5 text-[10px] text-[#e0e0e0] focus:outline-none focus:border-[#c8a96e] font-mono"
              >
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="flex-1 py-1.5 bg-[#c8a96e] text-black text-[9px] font-bold uppercase tracking-widest hover:bg-[#e0c080] disabled:opacity-50 transition-colors"
              >
                {isSavingCtx ? 'Saving…' : 'Save'}
              </button>
              <button
                onClick={() => setContextMenu(null)}
                className="px-3 py-1.5 border border-[#333] text-[#888] text-[9px] font-bold uppercase tracking-widest hover:border-[#555] hover:text-white transition-colors"
              >
                Cancel
              </button>
            </div>
